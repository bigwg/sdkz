...
func newUpgradeCmd() *cobra.Command {
	return &cobra.Command{
		Use:   "upgrade <candidate>",
		Short: "升级 SDK 到最新版本",
		Long: `安装该 SDK 当前可用的最新版本（按默认发行版/版本规格解析）。
...
示例:
  ...
  sdkz upgrade java`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			rel, err := m.Upgrade(cmd.Context(), candID, "", choose, p.OnProgress)
			if err != nil {
				if errors.Is(err, installer.ErrAlreadyInstalled) {
			...
			fmt.Fprintf(cmd.OutOrStdout(), "已升级 %s 到 %s\n", candID, rel.Version)
			return nil
		},
	}
}
